...
5. **Patterns**: 5a Task Mesh (persistent workers: task engine + `task_*` CLI/shim tools + dash) landed; 5b session handoff (`docs/handoff.md`, `DECISION_NEEDED` convention + dash decision inbox) landed; next: reviewer pair, tag fan-out, maybe datacopy file transfer. Live-status push via song classes/properties is the planned upgrade over today's polling.
